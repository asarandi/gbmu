...
bool server_create(char *network_address, int network_port) {
    ...
    if (bind(server.server_sock, (struct sockaddr *)&server.server_address, (socklen_t)
...
bool server_accept() {
    errno = 0;
    server.client_sock = accept(server.server_sock, (struct sockaddr *)&server.client_address,
                                (socklen_t * ) & server.client_address_len);
